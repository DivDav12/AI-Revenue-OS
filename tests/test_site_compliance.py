...
* internal links get the GitHub *project* Pages base path ("/<repo>/")
  when - and only when - a real deploy target is configured, so
  Impressum/Datenschutz are actually reachable on the deployed site
...
from revenue_os.ecosystem.model import POLICY_OK, SourceMeta
...
class BasePathTests(unittest.TestCase):
    ...
    def test_real_config_prefixes_every_internal_link(self):
        ...
        with mock.patch.dict("os.environ", _REAL_ENV, clear=True):
            html = site.render_homepage(_tmp())
            self.assertIn('href="/AI-Revenue-OS/impressum/"', html)
            self.assertIn('href="/AI-Revenue-OS/datenschutz/"', html)
            self.assertIn('href="/AI-Revenue-OS/kategorie/mikrofone/"', html)
    ...
    def test_guide_breadcrumb_is_prefixed_on_the_deployed_site(self):
        with mock.patch.dict("os.environ", _REAL_ENV, clear=True):
            page, _ = affiliate_assets.render_comparison_page(
                draft=_draft(), match=_amazon_match(), cta_url="https://x.test/go")
        self.assertIn('href="/AI-Revenue-OS/"', page)
        self.assertIn('href="/AI-Revenue-OS/kategorie/mikrofone/"', page)
